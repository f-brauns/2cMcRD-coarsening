...
for i,params in enumerate(parray):
    seed = params[1]
    #domain length
    L = params[2]
    Ltot += L
    ...
    #import data
    ef = h5py.File(foldername+"2cMcRD-rec-det-L-{:d}-dx-{:.1f}-Dc-10000-rhobar-{:.1f}-seed-{:d}-pbc".format(L,dl,rhobar,seed).replace(".","_")+".h5","r")
    ed = ef["data"]
    et = ef["times"]
    times = et[:]
...
if len(tts)>1:
    ...
    tt = tts[0]
#plot mean
ax1.plot(tt[::2],dl*Ltot/np.sum(peaknsn, axis=0)[::2], color="k", linestyle="None", marker="o", markersize=20, markerfacecolor='None', label="rhobar = {:.1f}".format(rhobar))   
...
ax1.plot(tt[:amax],dl*L/peaknsn[-1][norm]*1.2*(tt[:amax]/1e3)**(3/8), linewidth=4, label="~ t^(3/8)")

#plot numerical coarsening rate
ax1.plot(ts*scalef,1/(mu*rhobar)*masses, linewidth=4, linestyle="--")
