...
for k,ps in enumerate(parray):
    ...
    for i,params in enumerate(ps):
        ...
        #plot single run
        ax1.plot(rhobar*nu[k]*et[amin:],rhobar*dl*L/peakn[amin:], linewidth=1, color="grey")   
        tts.append(et[amin:])
    ...
    if len(tts)>1:
        ...
        tt = tts[0]
    #plot mean
    ax1.plot(rhobar*nu[k]*tt,rhobar*dl*Ltot/np.sum(peaknsn, axis=0), linewidth=3, color="k")   
    ...
    if k == 0:
        ax1.plot(rhobar*nu[k]*tt[:amax],rhobar*dl*L/peaknsn[-1][norm]*1.2*(tt[:amax]/1e3)**(3/8), linewidth=4,label = "~ t^(3/8)")
#plot numerical coarsening rate
ax1.plot(2/mu*ts,1/(mu)*masses, linewidth=4, linestyle="--", label="master curve")

#plot
ax1.set_title('recruitment - detachment kinetics, 1D', fontsize=26)
ax1.set_ylabel('average peak mass', fontsize=26)
ax1.set_xlabel('scaled time', fontsize=26)
...
plt.show(f)
f.savefig("../20200427-rhobar-collapse.pdf",format="pdf")