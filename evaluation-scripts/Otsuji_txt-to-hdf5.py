...
meta = {"PDE": "Otsuji model 2, only peaks",
        ...
        "Dm": Dm,
        "Dc": Dc,
        "n_initial": "rhobar*(1+0.01*rand(x))",
        "eta_initial": "etastar(n_initial)",
        "f(n,eta)": "n^2*eta-n"}
...
f = open("2cMcRD-Otsuji-rhobar-{:.1f}-L-{:d}-seed-{:d}-pbc".format(rhobar,L,seed).replace(".","_")+".txt")
...
#create export file
ef = h5py.File("2cMcRD-Otsuji-L-{:d}-dx-{:.1f}-rhobar-{:.1f}-seed-{:d}-pbc".format(L,dx[0],rhobar,seed).replace(".","_")+".h5","w-")
